...
def achievements():
    ...
    # カテゴリごとの料理タグを整理した辞書
    category_info = {
        'rice': ['rice', 'eels on rice', 'chicken-n-egg on rice', 'pork cutlet on rice', 'beef curry', 'chicken rice', 'fried rice', 'tempura bowl', 'sushi', 'bibimbap'],
        'bread': ['toast', 'croissant', 'roll bread', 'raisin bread', 'chip butty', 'hamburger', 'pizza', 'sandwiches'],
        'noodle': ['udon noodle', 'tempura udon', 'soba noodle', 'ramen noodle', 'beef noodle', 'tensin noodle', 'fried noodle', 'spaghetti', 'chilled noodle', 'dipping noodles'],
        'japanese_dishes': ['Japanese-style pancake', 'takoyaki', 'gratin', 'croquette', 'grilled eggplant', 'sauteed spinach', 'vegetable tempura', 'miso soup', 'potage', 'sausage', 'oden', 'omelet', 'ganmodoki', 'jiaozi', 'stew', 'teriyaki grilled fish', 'fried fish', 'grilled salmon', 'salmon meuniere', 'sashimi', 'grilled pacific saury', 'sukiyaki', 'sweet and sour pork', 'lightly roasted fish', 'steamed egg hotchpotch', 'tempura', 'fried chicken', 'sirloin cutlet', 'nanbanzuke', 'boiled fish', 'seasoned beef with potatoes', 'hambarg steak', 'beef steak', 'dried fish', 'ginger pork saute', 'spicy chili-flavored tofu', 'yakitori', 'cabbage roll', 'rolled omelet', 'egg sunny-side up', 'fermented soybeans', 'cold tofu', 'egg roll', 'stir-fried beef and peppers', 'simmered pork', 'boiled chicken and vegetables', 'sashimi bowl', 'sushi bowl', 'fish-shaped pancake with bean jam', 'shrimp with chill source', 'roast chicken', 'steamed meat dumpling', 'omelet with fried rice', 'cutlet curry', 'spaghetti meat sauce', 'fried shrimp', 'potato salad', 'green salad', 'macaroni salad', 'pork miso soup', 'chinese soup', 'beef bowl', 'kinpira-style sauteed burdock', 'rice ball', 'pizza toast'],
        'others': ['others', 'beverage'],
    }

    # タグ名ごとにアチーブメントを整理する辞書
    achievements = {
        'rice': {},
        'bread': {},
        'noodle': {},
    ...
    # ユーザーIDやタグ名をもとにアチーブメント情報を取得する
    for category, tag_list in category_info.items():
        for tag_name in tag_list:
            count = UserImageTag.count_tag_for_user(user_id, tag_name)  # ユーザーのタグごとのカウントを取得
            is_achieved = UserImageTag.check_achievement_for_tag(user_id, tag_name, target_count)  # アチーブメント達成かどうかを確認
            achievements[category][tag_name] = {
                'achieved_count': count,
                'is_achieved': is_achieved[0]
            }

    # テンプレートに渡す
    return render_template('detector/achievements.html', achievements=achievements)
...
def category_stats():
    ...
    for created_at, tag_name in tag_query:
        ...
        tag_calories = tag_info.get(tag_name, {}).get('category', "unknown")
        if date_key in date_category:
            date_category[date_key] +=1
        else:
            date_category[date_key] = date_category[date_key]
    
    dates = sorted(date_category.keys())
    ...
    for created_at, tag_name in tag_query:
        date_key = created_at.strftime('%Y-%m-%d') if created_at else "1970-01-01"
        tag_category = tag_info.get(tag_name, {}).get('category', 'unknown')  # categoryを取得

        # 日付ごとにカテゴリを管理
        if date_key not in date_category:
            date_category[date_key] = {}

        if tag_category in date_category[date_key]:
            date_category[date_key][tag_category] += 1  # 同じカテゴリーならカウントアップ
        else:
            date_category[date_key][tag_category] = 1  # 新しいカテゴリーなら1として初期化

    # 日付順にソートしてリスト化
    dates = sorted(date_category.keys())
    category_list = [date_category[date] for date in dates]
    ...
    user_image_tags = UserImageTag.query.filter_by(user_image_id=user_id).all()
    image_tags=[tag.tag_name for tag in user_image_tags]
...
def category_stats():
    ...
    calories_list ={tag: info['calories'] for tag, info in tag_info.items()}
    jyanru = {tag: info['category'] for tag, info in tag_info.items()}
